...
4. **Bearer authentication:** Enforces secure **Bearer Authentication** for all API communication between clients and servers, preventing unauthorized access to API resources.

5. **Granular API permissions:** Each API key can be scoped with specific permissions (e.g., `project_read`, `feedback_create`) to ensure principle of least privilege and provide fine-grained access control.
